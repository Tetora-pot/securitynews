    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if attempt < max_retries:
                print(f"[INFO] {cfg['name']}: retrying...", file=sys.stderr)
...
<body>
  ...
  <script>
    ...
    // Windows fix: use click (capture) to avoid change-event issues with btn-check inside overflow containers
    document.getElementById('articles-container').addEventListener('click', function(e) {{
      const label = e.target.closest('.checked-label');
      const cb = e.target.closest('.article-check') || (label && label.querySelector('.article-check'));
      if (!cb) return;
      e.preventDefault();
      cb.checked = !cb.checked;
      toggleChecked(cb.dataset.url);
    }}, true);
    ...
    // Windows fix: also trigger via click on filter labels (change may not fire inside overflow:auto on Windows)
